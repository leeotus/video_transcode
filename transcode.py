    ...
    if hdr_video:
        ...
        if input_file_video_codec == "hevc":  # use libx265 if hdr video
            ...
            hdr_kwargs.update(
                {
                    "vcodec": "libx265",
                    "crf": product_info["product_key"].get(
                        "h265_crf", BASE_CONF["crf"]
                    ),
                }
            )
...
def _ffmpeg_impl(input_file, out_kwargs, min_output_file_size, output_file_path):
    ...
    for _ in range(max_crf_adj_num):
        ...
        if (
            (not min_output_file_size)
            or int(new_info["format"]["size"]) > min_output_file_size
            or out_kwargs["crf"] <= 0
        ):
            break
        else:
...
def build_realtime_output_kwargs(input_video_info, profile, dst_width, dst_height):
    """Build output kwargs for realtime transcoding.

    Args:
        input_video_info (_type_): _description_
        profile (_type_): _description_
        dst_width (_type_): _description_
        dst_height (_type_): _description_
...
def spawn_ffmpeg_stream(input_url, out_kwargs):
    return (
        ffmpeg.input(input_url, noautorotate=None)
        .output("pipe:1", **out_kwargs)
